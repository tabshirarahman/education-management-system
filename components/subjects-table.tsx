"use client"

import { useEffect, useState } from "react"
import type { ISubject } from "@/types"
import { Button } from "@/components/ui/button"
import { Card } from "@/components/ui/card"
import { Edit2, Trash2 } from "lucide-react"

interface SubjectsTableProps {
  onEdit: (subject: ISubject) => void
  onDelete: (id: string) => void
  refreshTrigger: number
  departmentFilter?: string
}

export function SubjectsTable({ onEdit, onDelete, refreshTrigger, departmentFilter }: SubjectsTableProps) {
  const [subjects, setSubjects] = useState<ISubject[]>([])
  const [loading, setLoading] = useState(true)

  useEffect(() => {
    const fetchSubjects = async () => {
      try {
        const query = departmentFilter ? `?department=${departmentFilter}` : ""
        const res = await fetch(`/api/subjects${query}`)
        const data = await res.json()
        setSubjects(data.data || [])
      } catch (error) {
        console.error("Failed to fetch subjects:", error)
      } finally {
        setLoading(false)
      }
    }

    fetchSubjects()
  }, [refreshTrigger, departmentFilter])

  if (loading) return <div className="text-center py-8">Loading subjects...</div>

  return (
    <Card className="overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead>
            <tr className="border-b bg-slate-50">
              <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Subject Name</th>
              <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Code</th>
              <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Department</th>
              <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Credits</th>
              <th className="px-6 py-3 text-left text-sm font-semibold text-slate-900">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y">
            {subjects.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-6 py-8 text-center text-slate-500">
                  No subjects found. Create one to get started.
                </td>
              </tr>
            ) : (
              subjects.map((subject) => (
                <tr key={subject._id} className="hover:bg-slate-50">
                  <td className="px-6 py-4 text-sm font-medium text-slate-900">{subject.subjectName}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{subject.subjectCode}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{(subject.department as any)?.name || "N/A"}</td>
                  <td className="px-6 py-4 text-sm text-slate-600">{subject.credits}</td>
                  <td className="px-6 py-4 text-sm space-x-2 flex">
                    <Button size="sm" variant="outline" onClick={() => onEdit(subject)}>
                      <Edit2 className="w-4 h-4" />
                    </Button>
                    <Button size="sm" variant="destructive" onClick={() => onDelete(subject._id)}>
                      <Trash2 className="w-4 h-4" />
                    </Button>
                  </td>
                </tr>
              ))
            )}
          </tbody>
        </table>
      </div>
    </Card>
  )
}
