...
import { createResultSchema, type CreateResultInput } from "@/lib/validations/result"
...
export function ResultForm({ onSuccess, initialData, isEdit }: ResultFormProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [studentsRes, departmentsRes] = await Promise.all([fetch("/api/students"), fetch("/api/departments")])
  ...
  useEffect(() => {
    if (selectedDepartment) {
      const fetchSubjects = async () => {
        try {
          const res = await fetch(`/api/subjects?department=${selectedDepartment}`)
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select id="student" {...register("student")} className="w-full px-3 py-2 border rounded-md">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <Button type="button" size="sm" variant="outline" onClick={() => append({ subject: "", marks: 0 })}>
              Add Subject
            </Button>
          </div>
          ...
          {fields.length === 0 ? (
            ...
            <div className="space-y-3">
              {fields.map((field, index) => {
                ...
                return (
                  <div key={field.id} className="flex items-end gap-3 p-3 border rounded-lg bg-slate-50">
                    ...
                    <div className="w-20 text-center">
                      ...
                      <div className="font-semibold text-lg text-blue-600 mt-1">{grade}</div>
                    </div>
          ...
          {errors.subjectMarks && <p className="text-red-500 text-sm mt-2">{errors.subjectMarks.message}</p>}
        </div>
